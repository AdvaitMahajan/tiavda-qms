...
//
// Every notification email in the system is defined here, ONCE. Both the
// `send-email` edge function and any caller that names a template render from
// this file, so the look, branding and copy of every email lives in one place.
//
// To add a new notification email:
//   1. Add its key + params to the `TemplateParams` map below.
//   2. Add a render entry in `TEMPLATES`.
//   3. Call it from anywhere with  { template: 'your_key', params: {...} }.
//
// Editing copy/design = edit this file and redeploy `send-email`.
// ─────────────────────────────────────────────────────────────────────────────

// Brand / design tokens (mirrors src design system)
const BRAND = {
  navy: '#0F2A47',
  blue: '#1B5EA0',
  gold: '#D4930A',
...
// Company display name — overridable via SENDER_NAME secret at call time.
export const COMPANY_NAME =
  (typeof Deno !== 'undefined' && Deno.env.get('SENDER_NAME')) ||
  'Global Geotechnical Consultancy'

// Escape user-supplied text so names/notes can't break the HTML or inject markup.
...
// Shared branded shell every email is wrapped in — consistent header + footer.
function layout(opts: {
  heading: string
  subheading?: string
  body: string
  accent?: string // header background colour (defaults to navy)
}): string {
...
// Reusable key/value detail block.
...
// ── Param types for every template (frontend mirrors these keys) ──────────────
...
// ── The registry: one render function per notification ────────────────────────
const TEMPLATES: { [K in TemplateKey]: (p: TemplateParams[K]) => RenderedEmail } = {
  intake_link: (p) => ({
    subject: p.ref_number
      ? `Site Investigation Intake Form — ${p.ref_number}`
      : 'Site Investigation Intake Form',
    html: layout({
  ...
  intake_reminder: (p) => ({
    subject: p.is_final
      ? `Last Day: Intake form link expires today — ${p.ref_number}`
      : `Reminder: Please fill the intake form — ${p.ref_number}`,
    html: layout({
...
// Render a template by key. Throws if the key is unknown.
