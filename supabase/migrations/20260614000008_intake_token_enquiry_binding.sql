-- =============================================================================
-- Per-enquiry intake mapping: bind intake tokens to a specific enquiry so a
-- client with multiple open enquiries gets the intake attached to the RIGHT one.
-- Previously the token was client-bound and submit_intake_form attached to the
-- client's "most recent open enquiry" (fuzzy → wrong-enquiry attachment).
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS idx_intake_tokens_enquiry ON public.intake_tokens(enquiry_id);

-- Rewrite submit_intake_form to prefer the token's bound enquiry_id, falling back
-- to the old most-recent lookup only for legacy/client-level tokens.
...
BEGIN
  ...
  -- Target enquiry: the one the token is bound to (preferred), else the client's
  -- most-recent still-open enquiry (legacy fallback).
